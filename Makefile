...
apt-simphony-deps:
	...
	apt-get update -qq
	apt-get install -y python-dev cython libhdf5-7 libhdf5-serial-dev libatlas-dev libatlas3gf-base
	@echo
...
ifeq ($(USE_OPENFOAM_PARAVIEW),yes)
	echo deb http://www.openfoam.org/download/ubuntu precise main > /etc/apt/sources.list.d/openfoam.list
	apt-get update -qq
	apt-get install -y --force-yes paraviewopenfoam410 libhdf5-7 libhdf5-openmpi-dev
	@echo
	@echo "Paraview (openfoam) installed"
else
	apt-get install -y --force-yes paraview libhdf5-7 libhdf5-openmpi-dev
	@echo
